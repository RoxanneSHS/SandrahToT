...
CHAIN
IF~Global("ToLini","ara100",2)~THEN Tolini Pris1
...
++~ A drow to rescue from slavery, nothing we have not done before - with pleasure.~DO~SetGlobal("ToLini","ara100",3)~EXIT
++~ Drow or not drow, slavery is for nobody to endure, let us end this, friends.~ DO~ SetGlobal("ToLini","ara100",3)~EXIT
...
END
++~Fine, have it your way, you all. Attack.~DO~SetGlobal("ToLini","ara100",3)~EXIT

CHAIN
IF~Global("ToLini","ara100",4) ~THEN Tolini Pris2
...
END
++~Kelemvor spoke for all of us, come with us, Liriel.~DO~SetGlobal("ToLini","ara100",4)  ActionOverride("Tolini",JoinParty())~EXIT
++~Seems I have nothing to say in this case other than *yes*. So, welcome, Liriel.~ DO~SetGlobal("ToLini","ara100",4)  ActionOverride("Tolini",JoinParty())~EXIT